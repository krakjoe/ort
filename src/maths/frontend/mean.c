...
#define ORT_MATH_MEAN_AXIS_IMPL_FOR_TYPE(c_type, unused) \
    static zend_always_inline void ort_math_mean_axis_impl_##c_type( \
...
static zend_always_inline void ort_math_mean_axis_impl_zend_bool(
    void* result, const void *a, size_t outer, size_t axis, size_t inner) {
...
#define ORT_MATH_MEAN_IMPL_FOR_TYPE(c_type, unused) \
    static zend_always_inline void ort_math_mean_impl_##c_type( \
...
static zend_always_inline void ort_math_mean_impl_zend_bool(
    void* result, const void *a, size_t count) {
    zend_bool* va = (zend_bool*)a;
    zend_bool* res = (zend_bool*)result;
    size_t true_count = 0;
    for (size_t idx = 0; idx < count; idx++) {
        if (va[idx]) true_count++;
    }
    res[0] = (true_count >= (count / 2 + count % 2)) ? 1 : 0; // majority vote
}

ORT_MATH_FOREACH_NUMERIC_TYPE(ORT_MATH_MEAN_AXIS_IMPL_FOR_TYPE)
ORT_MATH_FOREACH_NUMERIC_TYPE(ORT_MATH_MEAN_IMPL_FOR_TYPE)

ort_tensor_t* ort_math_result_mean(ort_tensor_t* tensor, zval* axis_zval, zend_bool keepdims) {
    if (!ort_math_validate_input(tensor, "mean")) {
        return NULL;
    }
    ...
    if (Z_TYPE_P(axis_zval) != IS_LONG) {
        php_ort_status_throw(php_ort_status_tensor_invalidshape_ce, 
            "mean: axis must be an integer");
        return NULL;
    }

    int64_t axis = Z_LVAL_P(axis_zval);
    if (!ort_math_validate_axis(tensor, axis, "mean")) {
        return NULL;
    }
