...
/*
 @brief Implements frontend argmin operations for tensors
 @test tests/math/argmin
*/
...
ORT_MATH_FRONTEND_REDUCTION_AXIS_OP_DECL(argmin, float16) {
    ...
    float32 min = ort_math_float32_from_float16(va[flat0]);
    int64_t argmin = 0;
...
#define ORT_MATH_ARGMIN_AXIS_IMPL_FOR_TYPE(c_type, unused) \
    ORT_MATH_FRONTEND_REDUCTION_AXIS_OP_DECL(argmin, c_type) { \
        c_type* va = (c_type*)a; \
        int64_t* res = (int64_t*)result; \
        ORT_MATH_REDUCTION_KERNEL_AXIS_PROLOGUE() \
        \
...
ORT_MATH_FRONTEND_REDUCTION_AXIS_OP_DECL(argmin, zend_bool) {
...
ORT_MATH_FRONTEND_REDUCTION_OP_DECL(argmin, float16) {
    float16* va = (float16*)a;
    int64_t* res = (int64_t*)result;
...
#define ORT_MATH_ARGMIN_IMPL_FOR_TYPE(c_type, unused) \
    ORT_MATH_FRONTEND_REDUCTION_OP_DECL(argmin, c_type) { \
        c_type* va = (c_type*)a; \
        int64_t* res = (int64_t*)result; \
        c_type min = va[0]; \
        size_t argmin = 0; \
...
ORT_MATH_FRONTEND_REDUCTION_OP_DECL(argmin, zend_bool) {
    zend_bool* va = (zend_bool*)a;
    int64_t* res  = (int64_t*)result;
...
ORT_MATH_FOREACH_NUMERIC_TYPE(ORT_MATH_ARGMIN_AXIS_IMPL_FOR_TYPE)
ORT_MATH_FOREACH_NUMERIC_TYPE(ORT_MATH_ARGMIN_IMPL_FOR_TYPE)

static zend_always_inline int64_t* ort_math_frontend_reduce_shape_argmin(
    ort_tensor_t* tensor,
...
ORT_MATH_RESULT_REDUCE_AXIS_IMPL(argmin,
    ...
    ort_math_frontend_reduce_shape_argmin,
    &ort_math_promotion_schema_argmin);