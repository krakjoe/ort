...
/*
 * SIMD Subtraction Operations (SSE2)
 *
 * Only contracted types are implemented. SSE2 does not support 64-bit integer sub.
...
void ort_math_simd_sub_int64_t(void* result, const void* a, const void* b, size_t count) {
    ...
    ort_math_ops_sub_int64_t(result, a, b, count);
}
