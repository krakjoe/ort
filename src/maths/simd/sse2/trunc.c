...
/**
 * There are no SIMD truncation operations for float and double types in SSE2.
 */

void ort_math_simd_trunc_float(void* result, const void* a, size_t count) {
    ort_math_ops_trunc_float(result, a, count);
}

void ort_math_simd_trunc_double(void* result, const void* a, size_t count) {
    ort_math_ops_trunc_double(result, a, count);
}