...
void ort_math_simd_sign_float(void* result, const void* a, size_t count) {
    ...
    for (size_t i = 0; i < simd_count; i += simd_width) {
        ...
        __m128 gt = _mm_cmpgt_ps(va, zero);
        __m128 lt = _mm_cmplt_ps(va, zero);
        __m128 vr = _mm_or_ps(_mm_and_ps(gt, one), _mm_and_ps(lt, mone));
...
void ort_math_simd_sign_double(void* result, const void* a, size_t count) {
    ...
    for (size_t i = 0; i < simd_count; i += simd_width) {
        ...
        __m128d gt = _mm_cmpgt_pd(va, zero);
        __m128d lt = _mm_cmplt_pd(va, zero);
        __m128d vr = _mm_or_pd(_mm_and_pd(gt, one), _mm_and_pd(lt, mone));
