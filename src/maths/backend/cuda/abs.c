...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, abs, float16) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
        /* LCOV_EXCL_START */
        ORT_MATH_BACKEND_RELAY_CPU_UNARY(abs, FLOAT16)
            (res, va, count);
        return;
        /* LCOV_EXCL_STOP */
    }
...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, abs, float32) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
        /* LCOV_EXCL_START */
        ORT_MATH_BACKEND_RELAY_CPU_UNARY(abs, FLOAT32)
            (res, va, count);
        return;
        /* LCOV_EXCL_STOP */
    }
...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, abs, float64) {
    ...
    ort_cuda_abs_float64(res, va, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
        /* LCOV_EXCL_START */
        ORT_MATH_BACKEND_RELAY_CPU_UNARY(abs, FLOAT64)
            (res, va, count);
        return;
        /* LCOV_EXCL_STOP */
    }
