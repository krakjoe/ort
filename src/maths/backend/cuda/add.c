...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, float16) {
    const float16* va = (const float16*)a;
    const float16* vb = (const float16*)b;
    float16* res = (float16*)result;

    if (count * sizeof(float16) < __ort_cuda_threshold) {
        goto __ort_math_backend_add_float16_relay;
    }

    ort_cuda_add_float16(res, va, vb, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_float16_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
            __ort_math_cpu_dispatch, add, FLOAT16)
                (res, va, vb, count);
        return;
    }
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, float32) {
    ...
    if (count * sizeof(float32) < __ort_cuda_threshold) {
        goto __ort_math_backend_add_float32_relay;
    }

    ort_cuda_add_float32(res, va, vb, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_float32_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
            __ort_math_cpu_dispatch, add, FLOAT32)
                (res, va, vb, count);
        return;
    }
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, float64) {
    ...
    if (count * sizeof(float64) < __ort_cuda_threshold) {
        goto __ort_math_backend_add_float64_relay;
    }

    ort_cuda_add_float64(res, va, vb, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_float64_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
            __ort_math_cpu_dispatch, add, FLOAT64)
                (res, va, vb, count);
        return;
    }
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, int8_t) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_int8_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, int16_t) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_int16_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, int32_t) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_int32_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, int64_t) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_int64_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, uint8_t) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_uint8_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, uint16_t) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_uint16_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
...
ORT_MATH_BACKEND_BINARY_OP_DECL(cuda, add, uint32_t) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_add_uint32_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
