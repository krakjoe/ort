...
/* CUDA kernel for float16 reciprocal */
__global__ void ort_cuda_recip_float16_kernel(float16* result, const float16* a, size_t count) {
...
void ort_cuda_recip_float16(float16* result, const float16* a, size_t count, cudaStream_t stream) {
    ort_cuda_recip_float16_kernel<<<
