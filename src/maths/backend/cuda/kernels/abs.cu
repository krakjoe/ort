...
/* CUDA kernel for float absolute value */
__global__ void ort_cuda_abs_float_kernel(float* result, const float* a, size_t count) {
...
/* CUDA kernel for double absolute value */
__global__ void ort_cuda_abs_double_kernel(double* result, const double* a, size_t count) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
