...
/* CUDA kernel for float64 truncate */
__global__ void ort_cuda_trunc_float64_kernel(float64* result, const float64* a, size_t count) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void ort_cuda_trunc_float64(float64* result, const float64* a, size_t count, cudaStream_t stream) {
    ort_cuda_trunc_float64_kernel<<<
        ort_cuda_blocks_count(__ort_cuda_threads, count),
        __ort_cuda_threads, 0, stream>>>(
        result, a, count
    );
