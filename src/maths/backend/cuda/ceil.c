...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, ceil, float) {
    ...
    if (count * sizeof(float) < __ort_cuda_threshold) {
        goto __ort_math_backend_ceil_float_relay;
    }

    ort_cuda_ceil_float(res, va, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_ceil_float_relay:
...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, ceil, double) {
    ...
    if (count * sizeof(double) < __ort_cuda_threshold) {
        goto __ort_math_backend_ceil_double_relay;
    }

    ort_cuda_ceil_double(res, va, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_ceil_double_relay:
        ORT_MATH_BACKEND_RELAY(
