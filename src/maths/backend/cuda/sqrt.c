...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, sqrt, float16) {
    ...
    if (count * sizeof(float16) < __ort_cuda_threshold) {
        goto __ort_math_backend_sqrt_float16_relay;
    }

    ort_cuda_sqrt_float16(res, va, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_sqrt_float16_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
            __ort_math_cpu_dispatch, sqrt, FLOAT16)
                (res, va, count);
        return;
    }
...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, sqrt, float32) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_sqrt_float32_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, sqrt, float64) {
    ...
    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_sqrt_float64_relay: // LCOV_EXCL_LINE
        ORT_MATH_BACKEND_RELAY(
