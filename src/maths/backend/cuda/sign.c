...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, sign, float) {
    ...
    if (count * sizeof(float) < __ort_cuda_threshold) {
        goto __ort_math_backend_sign_float_relay;
    }

    ort_cuda_sign_float(res, va, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_sign_float_relay:
        ORT_MATH_BACKEND_RELAY(
            __ort_math_cpu_dispatch, sign, FLOAT)
                (res, va, count);
        return;
    }
...
ORT_MATH_BACKEND_UNARY_OP_DECL(cuda, sign, double) {
    ...
    if (count * sizeof(double) < __ort_cuda_threshold) {
        goto __ort_math_backend_sign_double_relay;
    }

    ort_cuda_sign_double(res, va, count, __ort_cuda_stream);

    if (cudaGetLastError() != cudaSuccess) {
__ort_math_backend_sign_double_relay:
        ORT_MATH_BACKEND_RELAY(
            __ort_math_cpu_dispatch, sign, DOUBLE)
                (res, va, count);
        return;
    }
