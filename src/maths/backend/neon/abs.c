...
ORT_MATH_BACKEND_UNARY_OP_DECL(neon, abs, int64_t) {
    ...
    if (mc == 0) {
        goto __ort_math_backend_abs_int64_fallback;
    }

    // Vectorized loop - process 2 int64_t at once using NEON
    for (size_t i = 0; i < mc; i += mw) {
        int64x2_t ma = vld1q_s64(&va[i]); 
        int64x2_t mr = vabsq_s64(ma);
        vst1q_s64(&res[i], mr);
    }

__ort_math_backend_abs_int64_fallback:
    if (mc < count) {
        ORT_MATH_FRONTEND_OP_SYMBOL(abs, int64_t)(
            res   + mc,
            va    + mc,
            count - mc);
    }
