...
ORT_MATH_BACKEND_UNARY_OP_DECL(avx2, round, float16) {
    ...
    if (mc == 0) {
        ...
        goto __ort_math_backend_round_float16_fallback;
    }
    ...
    for (size_t i = 0; i < mc; i += mw) {
        ...
        __m256 mf16r = _mm256_round_ps(mf16c,
...
__ort_math_backend_round_float16_fallback:
    ...
    if (mc < count) {
        ORT_MATH_FRONTEND_OP_SYMBOL(round, float16)(
            res   + mc,
            va    + mc,
            count - mc);
    }
...
/* SIMD round operation for float64 arrays */
