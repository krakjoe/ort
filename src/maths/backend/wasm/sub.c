...
ORT_MATH_BACKEND_BINARY_OP_DECL(wasm, sub, int8_t) {
    ...
    for (size_t i = 0; i < mc; i += mw) {
        ...
        v128_t mr = wasm_i8x16_sub(ma, mb);
...
ORT_MATH_BACKEND_BINARY_OP_DECL(wasm, sub, int16_t) {
    ...
    for (size_t i = 0; i < mc; i += mw) {
        v128_t ma = wasm_v128_load(&va[i]);
        v128_t mb = wasm_v128_load(&vb[i]);
        v128_t mr = wasm_i16x8_sub(ma, mb);
        wasm_v128_store(&res[i], mr);
    }
...
ORT_MATH_BACKEND_BINARY_OP_DECL(wasm, sub, int32_t) {
    ...
    for (size_t i = 0; i < mc; i += mw) {
        v128_t ma = wasm_v128_load(&va[i]);
        v128_t mb = wasm_v128_load(&vb[i]);
        v128_t mr = wasm_i32x4_sub(ma, mb);
        wasm_v128_store(&res[i], mr);
    }
...
ORT_MATH_BACKEND_BINARY_OP_DECL(wasm, sub, uint8_t) {
    ...
    const size_t mw = 16; // 16 uint8_t per 128-bit WASM SIMD
    ...
    // Vectorized loop - process 16 uint8_t at once using WASM SIMD
    for (size_t i = 0; i < mc; i += mw) {
        v128_t ma = wasm_v128_load(&va[i]);
        v128_t mb = wasm_v128_load(&vb[i]);
        // not a mistake, no unsigned sub, this is correct
        v128_t mr = wasm_i8x16_sub(ma, mb);
        wasm_v128_store(&res[i], mr);
    }
...
ORT_MATH_BACKEND_BINARY_OP_DECL(wasm, sub, uint16_t) {
    ...
    const size_t mw = 8; // 8 uint16_t per 128-bit WASM SIMD
    ...
    // Vectorized loop - process 8 uint16_t at once using WASM SIMD
    for (size_t i = 0; i < mc; i += mw) {
        v128_t ma = wasm_v128_load(&va[i]);
        v128_t mb = wasm_v128_load(&vb[i]);
        // not a mistake, no unsigned sub, this is correct
...
ORT_MATH_BACKEND_BINARY_OP_DECL(wasm, sub, uint32_t) {
    ...
    const size_t mw = 4; // 4 uint32_t per 128-bit WASM SIMD
    ...
    // Vectorized loop - process 4 uint32_t at once using WASM SIMD
    for (size_t i = 0; i < mc; i += mw) {
        v128_t ma = wasm_v128_load(&va[i]);
        v128_t mb = wasm_v128_load(&vb[i]);
        // not a mistake, no unsigned sub, this is correct
...
ORT_MATH_BACKEND_BINARY_OP_DECL(wasm, sub, float) {
    ...
    for (size_t i = 0; i < mc; i += mw) {
        ...
        v128_t mb = wasm_v128_load(&vb[i]);
        // not a mistake, no unsigned sub, this is correct
        v128_t mr = wasm_f32x4_sub(ma, mb);
        wasm_v128_store(&res[i], mr);
    }
