...
Some modern processors use a hybrid layout of performance and efficiency cores; often efficiency cores do not implement AVX2 (or others) at full speed, and if they implement it, they are doing so via emulation (or optimized for power efficiency) which may be slower than compiler optimized (for release builds) scalar code. While the backend is prepared to guard against the execution of illegal instructions; an E-Core with a dedicated SIMD pipeline will be allowed to execute optimized code (unless it's known to be problematic, such as gracemont), but will do so maximizing power efficiency not raw performance.

Where `ORT_SCALE_CORES` exceeds the number of cores available, `ORT_SCALE_CORES` will effectively be clamped to the number of cores available, this is done silently: The reason is that it may be too early in startup to raise errors/warnings gracefully. The alternative, to allow over subscription would create an unpredictable execution environment. This can only be misconfiguration, under normal conditions, omit to set `ORT_SCALE_CORES`, the maximum will be detected.

*The "number of cores available" is automatically detected, we use sysconf adjusted by the number of cores that would guard the CPU backend - ie, refuse to install it.*
