--TEST--
ORT\Math\sign: large vector, gpu
--EXTENSIONS--
ort
--SKIPIF--
<?php
...
// 1. sign for real types: negatives, zero, positives
...
foreach ($types as $name => [$type, $values]) {
    ...
    $data = $result->getData();
    echo "PASS: $name sign large tensor (gpu)\n";
    print_result($result);
}
?>
--EXPECTF--
PASS: FLOAT16 sign large tensor (gpu)
RESULT: %s
TYPE: FLOAT16
SHAPE: [8192]
PASS: FLOAT32 sign large tensor (gpu)
RESULT: %s
TYPE: FLOAT32
SHAPE: [4096]
PASS: FLOAT64 sign large tensor (gpu)
RESULT: %s
TYPE: FLOAT64
SHAPE: [4096]
