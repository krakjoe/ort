...
try {
    ...
    $test_types = [
        ['UNDEFINED', [0, 0], 'ONNX\Tensor::UNDEFINED'],
        ['FLOAT', [1.5, 2.5], 'ONNX\Tensor::FLOAT'],
        ['DOUBLE', [1.5, 2.5], 'ONNX\Tensor::DOUBLE'],
        ['INT8', [10, 20], 'ONNX\Tensor::INT8'],
        ['INT16', [100, 200], 'ONNX\Tensor::INT16'],
        ['INT32', [1000, 2000], 'ONNX\Tensor::INT32'],
        ['INT64', [10000, 20000], 'ONNX\Tensor::INT64'],
        ['UINT8', [50, 100], 'ONNX\Tensor::UINT8'],
        ['UINT16', [500, 1000], 'ONNX\Tensor::UINT16'],
        ['UINT32', [5000, 10000], 'ONNX\Tensor::UINT32']
    ];
    ...
    foreach ($test_types as $type_info) {
        ...
        try {
            ...
            $tensor_other = new ONNX\Tensor\Transient([2], $type_info[1], constant($type_info[2]));
            $result = ONNX\Math\add($tensor_bool, $tensor_other);
            
            // Test boolean as second operand  
            $tensor_other2 = new ONNX\Tensor\Transient([2], $type_info[1], constant($type_info[2]));
            $tensor_bool2 = new ONNX\Tensor\Transient([2], [false, true], ONNX\Tensor::BOOL);
            $result2 = ONNX\Math\add($tensor_other2, $tensor_bool2);
    ...
    try {
        ...
        $tensor_bool_max = new ONNX\Tensor\Transient([2], [true, false], ONNX\Tensor::BOOL);
        $tensor_int64_max = new ONNX\Tensor\Transient([2], [9223372036854775800, -9223372036854775800], ONNX\Tensor::INT64);
        $result = ONNX\Math\add($tensor_bool_max, $tensor_int64_max);
    ...
    try {
        ...
        $tensor_uint32_large = new ONNX\Tensor\Transient([3], [4294967290, 1000000000, 3000000000], ONNX\Tensor::UINT32);
        $result = ONNX\Math\add($tensor_bool, $tensor_uint32_large);
