--TEST--
ORT\Math\sqrt: large vector, gpu
--EXTENSIONS--
ort
--SKIPIF--
<?php
...
// 1. Sqrt for real types: negatives, zero, positives
...
foreach ($types as $name => [$type, $values]) {
    ...
    $data = $result->getData();
    echo "PASS: $name sqrt large tensor (gpu)\n";
    print_result($result);
}
?>
--EXPECTF--
PASS: FLOAT16 sqrt large tensor (gpu)
RESULT: %s
TYPE: FLOAT16
SHAPE: [4096]
PASS: FLOAT32 sqrt large tensor (gpu)
RESULT: %s
TYPE: FLOAT32
SHAPE: [4096]
PASS: FLOAT64 sqrt large tensor (gpu)
RESULT: %s
TYPE: FLOAT64
SHAPE: [4096]
