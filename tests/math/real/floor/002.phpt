--TEST--
ORT\Math\floor: large vector, gpu
--EXTENSIONS--
ort
--SKIPIF--
<?php
...
// 1. Floor for real types: negatives, zero, positives
...
foreach ($types as $name => [$type, $values]) {
    ...
    $result = ORT\Math\floor($a);
    $data = $result->getData();
    echo "PASS: $name floor large tensor (gpu)\n";
    print_result($result);
}
?>
--EXPECTF--
PASS: FLOAT16 floor large tensor (gpu)
RESULT: %s
TYPE: FLOAT16
SHAPE: [4096]
PASS: FLOAT32 floor large tensor (gpu)
RESULT: %s
TYPE: FLOAT32
SHAPE: [4096]
PASS: FLOAT64 floor large tensor (gpu)
RESULT: %s
TYPE: FLOAT64
SHAPE: [4096]
