--TEST--
ORT\Math\arccos: all types, shape, error handling, numpy semantics
--FILE--
<?php
use ORT\Tensor;
...
foreach ($real as $name => $type) {
    ...
    $result = ORT\Math\arccos($a);
    echo "PASS: $name arccos real [-1..1]\n";
    print_result($result);
}
?>
--EXPECTF--
PASS: FLOAT16 arccos real [-1..1]
RESULT: %s
TYPE: FLOAT16
SHAPE: [%d]
PASS: FLOAT32 arccos real [-1..1]
RESULT: %s
TYPE: FLOAT32
SHAPE: [%d]
PASS: FLOAT64 arccos real [-1..1]
RESULT: %s
TYPE: FLOAT64
SHAPE: [%d]
