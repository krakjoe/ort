--TEST--
ORT\Math\arcsinh: all types, shape, error handling, numpy semantics
--FILE--
<?php
use ORT\Tensor;
...
foreach ($real as $name => $type) {
    ...
    $result = ORT\Math\arcsinh($a);
    echo "PASS: $name arcsinh real [-5..5]\n";
    print_result($result);
}
?>
--EXPECTF--
PASS: FLOAT16 arcsinh real [-5..5]
RESULT: %s
TYPE: FLOAT16
SHAPE: [%d]
PASS: FLOAT32 arcsinh real [-5..5]
RESULT: %s
TYPE: FLOAT32
SHAPE: [%d]
PASS: FLOAT64 arcsinh real [-5..5]
RESULT: %s
TYPE: FLOAT64
SHAPE: [%d]
